...
<?php
	...
	function fetchStudentInfo($adm_no, $id_no){
		global $connection;
	...
	function getStudentByAdmNoAndIDNo($adm_no, $id_no){
		global $connection;
		$query = "SELECT * from students_tb where adm_no = '{$adm_no}' and id_no = '{$id_no}'";
	
		$student = mysqli_query($connection, $query);
		
		if($student){
			while ($res = mysqli_fetch_assoc($student)){
				return $res;
			}
		}
		else{
	...
	function postService($service_type, $provider_id, $charge, $area, $phone_no){
		global $connection;
		
		$query = "INSERT INTO services_tb(";
		$query .= "service_type, provider_id, charge, area, phone_no) ";
		$query .= "VALUES('{$service_type}', '{$provider_id}', '{$charge}', '{$area}', '{$phone_no}')";
